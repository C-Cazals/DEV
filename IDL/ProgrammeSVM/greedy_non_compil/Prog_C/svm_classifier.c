...
int *training_set_sampler_svm(char *in_dir, char *out_dir, char *area_file,
...
int main(int argc, char** argv)
{
	...
	char buffer[Max_Char],ColorMapTrainingSet16[Max_Char];
	...
	if ((strcmp(argv[1],"build_svm_config_file")== 0)){
		...
		if(strncmp(config_file,"/",1)==0){//We are on linux
			sprintf(in_dir, "/media/data/in_dir/T3/");
			sprintf(out_dir, "/media/data/out_dir/");
			...
			sprintf(in_dir, "d:\\data\\in_dir\\T3\\");
			sprintf(out_dir, "d:\\data\\out_dir\\");
	...
	if (argc > 10) { // Case of BATCH OFF (Polsarpro Interface)
	strcpy(batch, argv[1]);
	strcpy(polsarpro_directory, argv[2]);
	strcpy(file_script_svm, argv[3]);
	strcpy(in_dir, argv[4]);
	Bmp_flag = atoi(argv[5]);
	strcpy(ColorMapTrainingSet16, argv[6]);
	strcpy(config_file, argv[7]);
	strcpy(out_dir, argv[8]);
	strcpy(area_file, argv[9]);
	strcpy(cluster_file, argv[10]);
	strcpy(range_file, argv[11]);
	strcpy(cluster_file_norm, argv[12]);
	strcpy(svm_model_file, argv[13]);
	strcpy(classification_file, argv[14]);
	strcpy(Npt_max_classe, argv[15]);
	strcpy(unbalanced_training, argv[16]);
	strcpy(new_model, argv[17]);
	strcpy(cv, argv[18]);
	strcpy(log2c_begin, argv[19]);
	strcpy(log2c_end, argv[20]);
	strcpy(log2c_step, argv[21]);
	strcpy(log2g_begin, argv[22]);
	strcpy(log2g_end, argv[23]);
	strcpy(log2g_step, argv[24]);
	strcpy(kernel_type, argv[25]);
	strcpy(cost, argv[26]);
	strcpy(degree, argv[27]);
	strcpy(gamma, argv[28]);	
	strcpy(Npol, argv[29]);
	...
	for(i=0;i<Npolar;i++){
		input_ind_pol[i] = malloc(Max_Char * sizeof(char));
		strcpy(input_ind_pol[i], argv[i+30]);
	}
...
//	sprintf(cluster_file_all_path, "%s%s", in_dir, cluster_file);
	Npolar = atoi(Npol);
	
	if(strcmp(new_model,"0")==0){ // OLD SVM model file

		sprintf(classification_file, "%s_%s",classification_file, timing);// Addition of a date suffix to the classification file
		
		sprintf(file_name, "%s", config_file);
		sprintf(buffer, "%s_%s",config_file, timing);
		copy_file(file_name,buffer); // Copy of the temporary svm configuration file with a date suffix

		svm_predict_opt = 1;
		write_svm_script(polsarpro_directory, file_script_svm, svm_scale_opt, svm_cv_grid_opt,
			svm_train_opt, svm_predict_opt, in_dir, out_dir,range_file,
		...
		Npt_classe_out = training_set_sampler_svm(in_dir, out_dir, area_file, 
		...
		if(strcmp(kernel_type,"2")!=0){ // Case of Linear or Polynomial kernel
			sprintf(buffer, "%s_%s", config_file, timing);
			sprintf(config_file, "%s",buffer);// Addition of a date Suffix to the config file
			
			sprintf(buffer, "%s_%s", range_file, timing);
			sprintf(range_file, "%s",buffer);// Addition of a date Suffix to the config file
			
			sprintf(buffer, "%s_%s", svm_model_file, timing);
			sprintf(svm_model_file, "%s",buffer);// Addition of a date Suffix to the config file
			
			sprintf(buffer, "%s_%s", classification_file, timing);
			sprintf(classification_file, "%s",buffer);// Addition of a date Suffix to the config file
			
			// We wrtite the new config file with the addition time Suffix to the usefull file
			write_svm_conf_file(config_file, in_dir, out_dir, Npol,
				input_ind_pol, area_file,  cluster_file, range_file,
				cluster_file_norm, svm_model_file, classification_file,
				Npt_max_classe, unbalanced_training, new_model, cv, log2c_begin,
...
//void training_set_sampler_svm(char *cluster_file, int Npolar, int N_class,int *Npt_classe, int Npt_max_classe);
int *training_set_sampler_svm(char *in_dir, char *out_dir, char *area_file,
char *cluster_file, int Bmp_flag, char *ColorMapTrainingSet16,
int Npt_max_classe, int *Npt_classe, int Npolar, char **input_ind_pol)
{
...
/* INPUT/OUTPUT FILE OPENING*/
    for(i=0;i<Npolar;i++) {
		sprintf(file_name, "%s%s", in_dir,input_ind_pol[i] );
...
void random_sampling(char *cluster_file, int Npolar, int N_class,int *Npt_classe, int Npt_max_classe)
{
    ...
    char cluster_file_sampling[1024];
...
char *Npol, char **input_ind_pol)
{
	...
	char file_svm_grid[Max_Char];
	char file_python_exe[Max_Char];
	char prefix_exec[Max_Char];
	int Npolar,i, Max_pts = 0, num_class;
	
	if(strncmp(polsarpro_directory,"/",1)==0){//We are on linux
		sprintf(svm_gnuplot_path, "/usr/bin/gnuplot");
		sprintf(file_python_exe, "python");
		sprintf(svm_train_path, "%sSoft/SVM/svm_train_polsarpro", polsarpro_directory);
		sprintf(file_svm_grid, "%sSoft/SVM/grid_polsarpro.py", polsarpro_directory);
		sprintf(prefix_exec, "./");
	}else{//We are NOT on linux (probably Win32)
		sprintf(svm_gnuplot_path, "%sSoft\\lib\\pgnuplot.exe", polsarpro_directory);
		sprintf(file_python_exe, "%sSoft\\lib\\python", polsarpro_directory);
		sprintf(svm_train_path, "%sSoft\\SVM\\svm_train_polsarpro.exe", polsarpro_directory);
		sprintf(file_svm_grid, "%sSoft\\SVM\\grid_polsarpro.py", polsarpro_directory);
		sprintf(prefix_exec, " ");
	}
	
	sprintf(out_put_cv_txt, "%scross_val.txt", out_dir);
	sprintf(out_put_cv_png, "%scross_val.png", out_dir);
	...
	//Begining of the writting script process
	if(strncmp(polsarpro_directory,"/",1)==0){//We are on linux
	...
	if(svm_scale_opt == 1){
		...
		fprintf(file_script, "%ssvm_scale_polsarpro %s %s %s %s\n",
	...
	if(svm_cv_grid_opt == 1){
		...
		fprintf(file_script, "%sgrid_polsarpro -log2c %s %s %s -log2g %s %s %s -out %s -png %s -svmtrain %s -gnuplot %s %s %s %s\n",
		...
		svm_gnuplot_path, cluster_file_norm, svm_model_file, Npol);
			...
			fprintf(file_script, "%sgrid_polsarpro -log2c %s %s %s -log2g %s %s %s -out %s -png %s -svmtrain %s -gnuplot %s",
				prefix_exec,log2c_begin, log2c_end, log2c_step,log2g_begin,
	...
	if(svm_train_opt == 1){
		...
		if(strcmp(kernel_type,"0")==0){//Linear case
			if(strcmp(unbalanced_training,"0")== 0){// Unbalanced data DISABLE
			fprintf(file_script, "%ssvm_train_polsarpro -q -t 0 -c %s %s %s %s\n",
				prefix_exec,cost, cluster_file_norm, svm_model_file, Npol);
			}else{// Unbalanced data ENABLE
				fprintf(file_script, "%ssvm_train_polsarpro -q -t 0 -c %s",prefix_exec,cost);
				for(i=0;i<num_class;i++){//Loop to add the unbalanced settings
		...
		if(strcmp(kernel_type,"1")==0){//Polynomial case
			if(strcmp(unbalanced_training,"0")== 0){// Unbalanced data DISABLE
				fprintf(file_script, "%ssvm_train_polsarpro -q -t 1 -c %s -d %s %s %s %s\n",
					prefix_exec, cost, degree, cluster_file_norm, svm_model_file, Npol);
			}else{
				fprintf(file_script, "%ssvm_train_polsarpro -q -t 1 -c %s -d %s", prefix_exec, cost, degree);
				for(i=0;i<num_class;i++){//Loop to add the unbalanced settings
				fprintf(file_script, " -w%d %.3f",i+1, (float)Max_pts/(float)Npt_classe_out[i+1]);
				}
				fprintf(file_script, " %s %s %s\n", cluster_file_norm, svm_model_file, Npol);
		...
		if(strcmp(kernel_type,"2")==0){//RBF case
			if(strcmp(unbalanced_training,"0")== 0){// Unbalanced data DISABLE
				fprintf(file_script, "%ssvm_train_polsarpro -q -t 2 -c %s -g %s %s %s %s\n",
					prefix_exec, cost, gamma, cluster_file_norm, svm_model_file, Npol);
			}else{
				fprintf(file_script, "%ssvm_train_polsarpro -q -t 2 -c %s -g %s",prefix_exec, cost, gamma);
				for(i=0;i<num_class;i++){//Loop to add the unbalanced settings
				fprintf(file_script, " -w%d %.3f",i+1, (float)Max_pts/(float)Npt_classe_out[i+1]);
				}
				fprintf(file_script, " %s %s %s\n",
	...
	if(svm_predict_opt == 1){
		...
		fprintf(file_script, "%ssvm_predict_polsarpro %s %s %s %s %s",
			prefix_exec,in_dir, svm_model_file, range_file, classification_file, Npol); //Name of the output classification
		for(i=0;i<Npolar;i++){
